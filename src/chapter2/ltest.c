...
int InitRawSocket(char *device, int promiscFlag, int ipOnly) {
    ...
    */
    ...
    // ioctl & SIOCGIFINDEX : get interface index of the interface and give it to ifr_ifindex
    ...
    if(bind(soc, (struct sockaddr *)&sa, sizeof(sa))<0) { //bind: decide interface
        perror("bind");
        close(soc);
        return -1;
    }
...
// my_ether_ntoa_addr(): MAC address -> string
char *my_ether_ntoa_r(u_char *hwaddr, char *buf, socklen_t size) {
    printf("in my_ether_ntoa_r\n");
