...
PARAM Param = {"eth0", "eth1", 1, "192.168.144.2"}; //input
...
int SendIcmpTimeExceeded(int deviceNo, struct ether_header *eh, struct iphdr *iphdr, u_char *data, int size) {
    ...
    memcpy(reh.ether_dhost, eh->ether_shost, 6); //destination mac address <- source
    memcpy(reh.ether_shost, Device[deviceNo].hwaddr, 6); //source mac address <- router
